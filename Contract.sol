...
// True if the lottery is running
 uint256 public ticketCost;
 address public nftContract;
// Address of the NFT contract for the prize
uint256 public tokenId;
// Token ID of the NFT for the prize
 uint256 public totalEntries;
// Total number of entries
event NewTicketBought(address player);
// Event when someone buys a ticket
 event LotteryStarted();
event LotteryEnded();
event Winner(address winner);
// Eventwhen someone wins the lottery
 event TicketCostChanged(uint256 newCost);
// Eventwhen the ticket cost is updated
...
// Sets the admin as the only one who can call functions _;
}
...
// Emit the event that a new ticket was bought
 }
...
// Lottery must not be running
 require(nftContract == address(0),"Prize from previous lottery not transferred");
require(
     ERC721Base(_nftContract).ownerOf(_tokenId) == admin,"Admin does not own the specified NFT.");
// Admin must own the NFT
nftContract = _nftContract;
tokenId = _tokenId;
lotteryStatus = true;
//Set the lottery status to true
 emit LotteryStarted();
// Emit the event that the lottery has started 
...
// Lottery must be running
 lotteryStatus = false;
//Set the lottery status to false
 emit LotteryEnded();
// Emit the event that the lottery has ended 
}
...
resetEntryCounts();
// Reset the entry counts
 delete playerSelector;
// Reset the playerSelectorarray
 delete players;
// Reset the players array
 lotteryStatus = false;
//Set lottery status to completed
 nftContract = address(0);
// Reset the NFT contract address
 tokenId = 0;
// Reset the token ID
 totalEntries = 0;
// Reset the total entries
 }
function changeTicketCost(uint256 _newCost) public onlyAdmin {
     require(! lotteryStatus, "Lottery is still running");
// Lottery must not be running
 ticketCost = _newCost;
//Set the new ticket cost
 emit TicketCostChanged(_newCost);
// Emit the event that the ticket cost was updated
 } 
...
// Return the contract balance
 } function withdrawBalance() public onlyAdmin { require(address(this).balance > 0,"No balance to withdraw.");
uint256 amount = address(this).balance;
payable(admin).transfer(amount);
emit BalanceWithdrawn(amount);
} // Master reset function to reset the contract
 function resetContract() public onlyAdmin { delete playerSelector;
// Reset the playerSelector array delete players;
// Reset the players array
 lotteryStatus = false;
//Set lottery status to completed
 nftContract = address(0);
