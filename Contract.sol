...
event LotteryEnded();
event Winner(address winner);// Eventwhen someone wins the lottery
 event TicketCostChanged(uint256 newCost);// Eventwhen the ticket cost is updated
...
modifier onlyAdmin() {
     require(msg.sender == admin, "Only admin can call this function");// Sets the admin as the only one who can call functions _;
}
...
require(
     ERC721Base(_nftContract).ownerOf(_tokenId) == admin,"Admin does not own the specified NFT.");// Admin must own the NFT
...
resetEntryCounts();// Reset the entry counts
 delete playerSelector;// Reset the playerSelectorarray
